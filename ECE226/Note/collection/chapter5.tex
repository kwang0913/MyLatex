...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item Uncorrelated is linear independent. But independent includes both linear and nonlinear independent. Thus, uncorrelatedness does not imply independence. \eg, $X\sim \Univ[-1,1]$ and $Y=X^2$.
        ...
        \item Joint Gauvsian Random Variables has a preferred property: uncorrelatedness $\iff$ independence. This is the fundamental of a lot of (I would say more than 85\%) stochastic modeling, machine learning, etc.
    \end{enumerate}
\end{theorem}


\section{Bivariate Gauvsian Random Variables}
\begin{definition}
    [Bivariate Gauvsian Random Variables]
    Random variables $X$ and $Y$ are bivariate Gauvsian if and only if their joint PDF is given by the following equation,
...
\begin{theorem}
    If $X$ and $Y$ are bivariate Gauvsian, then $X$ is the $\mathsf{N}(\mu_X, \sigma_X)$ and $Y$ is the $\mathsf{N}(\mu_Y, \sigma_Y)$.
\end{theorem}

\begin{theorem}
    Bivariate Gauvsian Random Variables are uncorrelated if and only if they are independent.
\end{theorem}

\begin{theorem}
    If $X$ and $Y$ are bivariate Gauvsian, and $W_1$ and $W_2$ are given by linear independent equations,
    \[W_1=a_1X+b_1Y, \qquad W_2=a_2X+b_2Y,\]
    then $W_1$ and $W_2$ are bivariate Gauvsian random variables such that
    \begin{align*}
